...
#[pyfunction]
pub fn calcsize(format: &str) -> PyResult<usize> {
    ...
    if let Some('@') = it.next() {
        ...
        it.try_fold(0_usize, |acc, x| match x {
            ...
            _ => Err(PyValueError::new_err("bad char in struct format")),
        })
    } else {
        // Standard size
...
#[pyclass(subclass, module = "scapy.core.struct")]
pub struct Struct {
    order: StructOrder,
    value: StructValue,
}
...
impl Struct {
    pub fn _to_bytes(&self) -> Box<[u8]> {
        match self.order {
            StructOrder::NativeOrderNative => match self.value {
                StructValue::Char(x) => {
            ...
            StructOrder::NativeOrderStandard => match self.value {
            ...
            StructOrder::LittleEndianStandard => match self.value {
            ...
            StructOrder::BigEndianStandard => match self.value {
...
impl Struct {
    pub fn to_bytes(&self, py: Python) -> PyObject {
        PyBytes::new(py, &self._to_bytes()).into()
    }
}

#[pyfunction]
pub fn pack(format: &str, val: PyObject) -> PyResult<Struct> {
...
#[pymodule]
pub fn r#struct(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    m.add_class::<Struct>()?;
    m.add_function(wrap_pyfunction!(calcsize, m)?)?;
