...
values.
        ...
        # scls can be packet, str (packet or packet+field)
        # ccls can be packet, str (packet only)
        if isinstance(cls, int):
            nb = cls + 1
            scls = ""  # type: Union[Type[Packet], str]
        else:
            scls = cls
        ccls = ""  # type: Union[Type[Packet], str]
        fld = None  # type: Optional[str]
        if isinstance(scls, str) and "." in scls:
            ccls, fld = scls.split(".", 1)
        else:
            ccls, fld = scls, None
        if not ccls or match(self.__class__, ccls) \
        ...
        for f in self.packetfields:
            ...
            for fvalue in fvalue_gen:
                if isinstance(fvalue, Packet):
                    track = []  # type: List[int]
                    ret = fvalue.getlayer(ccls, nb=nb, _track=track,
                                          _subclass=_subclass, **flt)
                    if ret is not None:
                        return ret
                    nb = track[0]
        return self.payload.getlayer(ccls, nb=nb, _track=_track,
                                     _subclass=_subclass, **flt)
